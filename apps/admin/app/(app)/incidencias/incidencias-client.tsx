...
export function IncidenciasClient({ incidenciasIniciales }: { incidenciasIniciales: Incidencia[] }) {
  ...
  const [detalleAbierto, setDetalleAbierto] = useState<{ incidenciaId: string; paqueteId: string } | null>(null);
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {visibles.map((i) => (
              <tr
                key={i.id}
                onClick={() => setDetalleAbierto({ incidenciaId: i.id, paqueteId: i.paqueteId })}
                className="cursor-pointer hover:bg-muted/40"
              >
                {/* El módulo Incidencias es autónomo: la fila completa
                    abre el detalle (igual que el botón "Ver detalle"),
                    y el único camino hacia /paquetes es el botón
                    explícito "Ir al paquete" dentro de Acciones — nunca
                    un clic sobre el nombre/unidad del residente. */}
                <td className="px-4 py-2.5">
                  <p className="font-medium">{i.unidadIdentificador}</p>
                ...
                <td className="px-4 py-2.5">
                  <div className="flex items-center gap-3" onClick={(e: MouseEvent) => e.stopPropagation()}>
                    <button
                      onClick={() => setDetalleAbierto({ incidenciaId: i.id, paqueteId: i.paqueteId })}
